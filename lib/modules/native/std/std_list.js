...
module.exports = {
    ...
    "std:indexOf": function(args) {
        if(args.length > 1 && args[0] instanceof Blobs.ListBlob && args[1] instanceof Blobs.Blob) {
            return new Blobs.NumberBlob(_.findIndex(args[0].getItems(), args[1]));
        }
    },

    "std:union": function(args) {
        if(_.every(_.map(args, arg => arg instanceof Blobs.ListBlob))) {
            let items = _.map(args, arg => arg.getItems());
            items = _.values(items);
            return new Blobs.ListBlob(_.values(_.unionWith(...items, _.isEqual)));
        }
    },
