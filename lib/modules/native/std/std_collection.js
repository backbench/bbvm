...
function nestedGet(currentMap, currentKeys) {
    if(currentKeys.length === 0) {
        return currentMap;
    } else {
        let possibleKeys = [];
        for(let keysLength = 1; keysLength <= currentKeys.length; keysLength++) {
            possibleKeys.push(currentKeys.slice(0, keysLength).join("."));
        }
        for(let possibleKey of possibleKeys) {
            if(currentMap instanceof Blobs.MapBlob && currentMap.mapRep().hasOwnProperty(possibleKey)) {
                let result = nestedGet(currentMap.mapRep()[possibleKey], _.difference(currentKeys, possibleKey.split(".")));
                if(result)
                    return result;
            }
        }
...
module.exports = {
    ...
    "std:get": function(args) {
        ...
        if(args.length > 1 && args[0] instanceof Blobs.MapBlob && args[1] instanceof Blobs.StringBlob) {
            return nestedGet(args[0], args[1].value().split("."));
