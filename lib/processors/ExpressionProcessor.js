...
var processAssignment = function(l, value, localContext, globalContext) {
    ...
    } else if(l.type === Types.EXPRESSION_MEMBER_DOT || l.type === Types.EXPRESSION_MEMBER_INDEX) {
        let nestedExpressions = [];
        let keyIterator = l;
        let isValidAddress = true;
        while(keyIterator) {
            if(keyIterator.type === Types.EXPRESSION_MEMBER_INDEX) {
                nestedExpressions.unshift(processExpression(keyIterator.index, localContext, globalContext));
            } else if(keyIterator.type === Types.EXPRESSION_MEMBER_DOT) {
                nestedExpressions.unshift(new Blobs.StringBlob(keyIterator.key));
            } else if(keyIterator.type === Types.EXPRESSION_ID) {
                nestedExpressions.unshift(keyIterator);
            } else {
                isValidAddress = false;
                break;
            }
            keyIterator = keyIterator.root;
        }

        isValidAddress && Promise.all(nestedExpressions).then(function(nestedKeys) {
            utils.write(nestedKeys, value, localContext);
        });
    }
...
var expressionProcessors = {
    ...
    [Types.EXPRESSION_UNARY_POSTFIX_OP]: function(e, localContext, globalContext) {
        return new Promise(function(done) {
            processExpression(e.operand, localContext, globalContext).then(function(operand) {
                if(e.operator === "++" && operand instanceof Blobs.NumberBlob) {
                    let incValue = new Blobs.NumberBlob(operand.numberRep() + 1);
                    processAssignment(e.operand, incValue, localContext, globalContext);
                    done(operand);
                } else if(e.operator === "--" && operand instanceof Blobs.NumberBlob) {
                    let decValue = new Blobs.NumberBlob(operand.numberRep() - 1);
                    processAssignment(e.operand, decValue, localContext, globalContext);
                    done(operand);
                } else {
                    done(Constants.BETAL_NULL);
                }
        ...
        return new Promise(function(done) {
            processExpression(e.left, localContext, globalContext).then(function(left) {
                if (e.operator === "+") {
                    ...
                    processExpression(e.right, localContext, globalContext).then(function(right) {
                        done(binaryOps.binaryOpLeftShift(left, right));
                    });
                } else if(e.operator === ">>") {
                    processExpression(e.right, localContext, globalContext).then(function(right) {
                        done(binaryOps.binaryOpRightShift(left, right));
                    });
                } else {
                    done(Constants.BETAL_NULL);
                }
        ...
        return new Promise(function(done) {
            processExpression(e.left, localContext, globalContext).then(function(left) {
                processExpression(e.right, localContext, globalContext).then(function(right) {
                    let value = right;
                    if(e.operator === "=") {
                        processAssignment(e.left, right, localContext, globalContext);
                    } else if(e.operator === "+=") {
                        value = processAssignment(e.left, binaryOps.binaryOpPlus(left, right), localContext, globalContext);
                    } else if(e.operator === "-=") {
                        value = processAssignment(e.left, binaryOps.binaryOpMinus(left, right), localContext, globalContext);
                    } else if(e.operator === "*=") {
                        value = processAssignment(e.left, binaryOps.binaryOpMultiply(left, right), localContext, globalContext);
                    } else if(e.operator === "/=") {
                        value = processAssignment(e.left, binaryOps.binaryOpDivide(left, right), localContext, globalContext);
                    } else if(e.operator === "%=") {
                        value = processAssignment(e.left, binaryOps.binaryOpModulo(left, right), localContext, globalContext);
                    }
                    done(value);
                });
            });
