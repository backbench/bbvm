...
var expressionProcessors = {
    ...
    [Types.EXPRESSION_ASSIGNMENT]: function(e, localContext, globalContext) {
        return new Promise(function(done) {
            ...
            if(e.left.type == Types.EXPRESSION_MEMBER_DOT) {

                processExpression(e.right, localContext, globalContext).then(function(value) {
                    let result;
                    if(e.operator === "=") {
                        
                        let nested_roots = e.left;
                        result = new Blobs.MapBlob({[nested_roots.key]: value});

                        while(nested_roots.root.type === Types.EXPRESSION_MEMBER_DOT) {
                            nested_roots = nested_roots.root;
                            result = new Blobs.MapBlob({ [nested_roots.key]: result});
                        }

                        processAssignment(nested_roots.root, result, localContext, globalContext);
                    }
                    done(result);
                });
