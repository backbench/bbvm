...
var statementProcessors = {
    ...
    [Types.STATEMENT_BLOCK]: function(s, localContext, globalContext) {
        return processStatements(s.statements);
    },
    ...
    [Types.STATEMENT_FOR_IN]: function(s, localContext, globalContext) {
        return new Promise(function(done) {
            ExpressionProcessor.processExpression(s.collection).then(function(collection) {
    ...
    [Types.STATEMENT_RETURN]: function(s, localContext, globalContext) {
        return new Promise(function(done) {
            if (s.value) {
                ExpressionProcessor.processExpression(s.value).then(function(value) {
                    done(RETURN(value));
                });
            } else {
                done(RETURN());
