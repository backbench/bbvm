...
class BetalBlob extends Blob {
    constructor(value) {
        ...
        if (_.isString(value) && _.includes([STR_NULL, STR_FALSE, STR_TRUE], value.toLowerCase())) {
            ...
            this._wrapped = STR_NULL;
        } else if(_.isBoolean(value)) {
            if(value) {
                this._wrapped = STR_TRUE;
            } else {
                this._wrapped = STR_FALSE;
            }
        }
        else {
    ...
    json() {
        if(this._wrapped === STR_NULL) {
            return null;
        } else if(this._wrapped === STR_FALSE) {
            return false;
        } else if(this._wrapped === STR_TRUE) {
            return true;
        }
    }
